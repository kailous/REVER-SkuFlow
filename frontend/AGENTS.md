...
A mechanism library may contain multiple fixed mechanisms. During library creation, an optional `固定机制` switch opens a required second step for adding one or more fixed mechanisms, each with its own paragraph copy and gift combination. Library detail separates fixed and ordinary mechanisms. Every fixed mechanism is automatically additive for every SKU in every activity using that library; users may still select one ordinary mechanism per SKU. Copying a mechanism or whole library preserves whether each mechanism is fixed.
